...
SoundManager::SoundManager()
{
	...
	//when the player is 50 pixels away sound if max
	float minDistance = 150;

	//Sound reduces as the player moves away
	float attenuation = 15;

	//set the attenuation levels
	...
	Vector2f emitterLocation, Vector2f listenerLocation)
{
	...
	//Go back to 1 when sound 3 has started
	if (m_NextSound > 3)
	{
		m_NextSound = 1;
	}
